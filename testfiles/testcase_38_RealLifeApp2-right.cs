...
namespace Logic.UI.ViewModels
{
  public class MainViewModel : ObservableObject
  {
    ...
    public MainViewModel()
    {
      ...
      MediaLibrary = new MediaLibraryViewModel(UiTools);
      MediaLibrary.Path = @"D:\Medien\Tonträger";

      // Create a collection of PageViewModels and add all needed pages
      PageViewModels = new List<PageViewModelBase>
      {

        // Add the pages
        new BiographyViewModel(MediaLibrary, UiTools),
        new LyricsPercentageViewModel(MediaLibrary, UiTools),
        new SettingsViewModel(MediaLibrary, UiTools)
      };
      ...
      // Define a command that will be run when scanning the media
      // library is finished
      var cmdScanningFinished = new RelayCommand(() =>
      {
        UiTools.StatusBar.StatusText = $"{MediaLibrary.Artists.Count} artists loaded.";
      });

      // Start scanning the media library
      UiTools.StatusBar.StatusText = "Scanning media library..";
      UiTools.LoadingPanelViewModel.BusyMessage = "Scanning media library..";

      MediaLibrary.ScanAsync(cmdScanningFinished, false);
    }
  }
}
